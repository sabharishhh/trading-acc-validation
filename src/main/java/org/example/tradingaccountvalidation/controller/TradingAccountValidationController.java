...
public class TradingAccountValidationController {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(TradingAccountValidationController.class);

    @Autowired
    private TradingAccountValidationInterface service;

    @PostMapping("/user")
    public ResponseEntity<DynamicAccountSnapshot> validateAccount(
        ...
        try {
            DynamicAccountSnapshot updatedSnapshot =
                    service.validateAccount(snapshot);
